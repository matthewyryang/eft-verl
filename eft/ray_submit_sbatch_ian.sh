...
#SBATCH --output=/u/iwu2/logs/slurm-ray-%j.out
#SBATCH --error=/u/iwu2/logs/slurm-ray-%j.err  # Good practice for separate error logs
#SBATCH --requeue
...
JOB_WORKING_DIR="/u/iwu2/code/eft-verl"
# Define the script to run *relative to the working directory*
JOB_SCRIPT_NAME="/u/iwu2/code/eft-verl/eft/grpo/run_interventions_e3.sh"
...
srun --export=ALL --nodes=1 --ntasks=1 -w "$head_node" \
    /u/iwu2/anaconda3/envs/verl/bin/ray start --head --node-ip-address="$head_node_ip" --port=$RAY_PORT --dashboard-host=0.0.0.0 --dashboard-port=$RAY_DASHBOARD_PORT \
...
for worker_node in "${worker_nodes[@]}"; do
    ...
    srun --export=ALL --nodes=1 --ntasks=1 -w "$worker_node" \
        /u/iwu2/anaconda3/envs/verl/bin/ray start --address="$head_node_ip:$RAY_PORT" \
        --num-cpus=$SLURM_CPUS_PER_TASK --num-gpus=$SLURM_GPUS_ON_NODE \
        --block &
...
/u/iwu2/anaconda3/envs/verl/bin/ray status || echo "WARNING: Ray status check failed or cluster not fully ready yet."
sleep 5
...
#     /project/flame/yuxiaoq/miniconda3/envs/verl/bin/ray job submit --address="http://127.0.0.1:8265"  --no-wait --runtime-env $JOB_WORKING_DIR/runtime_env.yaml sh -c "cd $JOB_WORKING_DIR && exec bash $JOB_SCRIPT_NAME")
